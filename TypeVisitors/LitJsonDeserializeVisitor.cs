...
{
    internal class LitJsonDeserializeVisitor: ITypeFuncVisitor<string, string, int, string>
    {
        ...
        public string Accept(TArray type, string json, string x, int depth)
        {
            ...
            return $"{{ var {__json} = {json}; int {_n} = ({__json} as JsonData).Count; {x} = new {typeStr}; int {__index}=0; foreach(JsonData {__e} in {__json}) {{ {type.ElementType.Apply(DeclaringTypeNameVisitor.Ins)} {__v};  {type.ElementType.Apply(this, $"{__e}", $"{__v}", depth + 1)}  {x}[{__index}++] = {__v}; }}   }}";
        }
        ...
        public string Accept(TList type, string json, string x, int depth)
        {
            ...
            string __v = $"__v{depth}";
            string __json = $"__json{depth}";
            return $"{{ var {__json} = {json}; {x} = new {type.Apply(DeclaringTypeNameVisitor.Ins)}(({__json} as JsonData).Count); foreach(JsonData {__e} in {__json}) {{ {type.ElementType.Apply(DeclaringTypeNameVisitor.Ins)} {__v};  {type.ElementType.Apply(this, $"{__e}", $"{__v}", depth + 1)}  {x}.Add({__v}); }}   }}";
        }
        ...
        public string Accept(TMap type, string json, string x, int depth)
        {
            ...
            string __v = $"_v{depth}";
            string __json = $"__json{depth}";
            return @$"{{ var {__json} = {json}; {x} = new {type.Apply(DeclaringTypeNameVisitor.Ins)}(({__json} as JsonData).Count); foreach(JsonData {__e} in {__json}) {{ {type.KeyType.Apply(DeclaringTypeNameVisitor.Ins)} {__k};  {type.KeyType.Apply(this, $"{__e}[0]", __k, depth + 1)} {type.ValueType.Apply(DeclaringTypeNameVisitor.Ins)} {__v};  {type.ValueType.Apply(this, $"{__e}[1]", __v, depth + 1)}  {x}.Add({__k}, {__v}); }}   }}";
        }
    }
}
